...
//	Lm 
	for(int it=0; it<fdtdgpu.nt; ++it)    
    	{
        	fdtdgpu.elastic_rotation_addS_for(it,0);		//add source

			fdtdgpu.born_modeling(direction,1);			// 1st born modeling
			fdtdgpu.born_modeling(direction,5);			// 2nd born modeling

			fdtdgpu.record_scatter(it,doublescatter);		//record wavefield
		if(!rbc){
			if(!cpu_mem){
...
//					std::cout<<it<<std::endl;
			...
			fdtdgpu.born_modeling(direction_rec,6);			//reconstruct background wavefield
			fdtdgpu.born_modeling(direction_rec,7);			//reconstruct scatter wavefield

// backward wavefield
        	fdtdgpu.elastic_rotation_addS_bac(it,0,stack);	// add adjoint source
			fdtdgpu.born_modeling(direction,8);			// adjoint wavefield
			fdtdgpu.born_modeling(direction,9);			// adjoint scatter wavefield

	 		cudaDeviceSynchronize();			
			fdtdgpu.image_gradient(0);				//calculate gradient
