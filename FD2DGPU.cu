...
// 	__constant__ float coeff1[4]={1225.0/1024.0,-245.0/3072.0,49.0/5120.0,-5.0/7168.0};
// #endif
...
#pragma unroll 4
		...
		// vx_gx[iz+ix*nzpml] = (vx[(ix)*nzpml+iz]-vx[(ix-1)*nzpml+iz])/dx;
		// vz_gz[iz+ix*nzpml] = (vz[ix*nzpml+(iz)]-vz[ix*nzpml+(iz-1)])/dz;
...
template<typename T>
__global__ void fft_ifft_shift(T *data, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void construct_filter(int nx,int nz,int x1,int x2,int z1,int z2,float* filter)
{
	int ix,iz;
	...
	if(ix<x1||ix>nx-x1||iz<z1||iz>nz-z1||(ix>w2-x2&&ix<w2+x2&&iz>h2-z2&&iz<h2+z2)){
		filter[ix*nz+iz] = 0;
	}
	else{
		filter[ix*nz+iz] = 1;		
...
// template<typename T>
// void gaussian_bandpass_filter(T *filter, int width, int height, int low_cutoff, int high_cutoff) {
//     int cx = width / 2;
//     int cy = height / 2;
...
//             T gaussian = exp(-distance * distance / (2.0 * high_cutoff * high_cutoff)) - exp(-distance * distance / (2.0 * low_cutoff * low_cutoff));
//             if (gaussian < 0) {
//                 gaussian = 0;
//             }
...
template<typename T>
void construct_gaussian_bandpass_filter(T *filter, int width, int height, int low_cutoff_x, int high_cutoff_x, int low_cutoff_y, int high_cutoff_y) {
    int cx = width / 2;
    int cy = height / 2;
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            T gaussian_x = exp(-dx * dx / (2.0 * sigma_hx * sigma_hx)) - exp(-dx * dx / (2.0 * sigma_lx * sigma_lx));
            T gaussian_y = exp(-dy * dy / (2.0 * sigma_hy * sigma_hy)) - exp(-dy * dy / (2.0 * sigma_ly * sigma_ly));
...
void bandpass_filter(int nx,int nz,int x1,int x2,int z1,int z2,float* bgrad_pp,float* bgrad_ps){
    ...
    // construct_gaussian_bandpass_filter(gauss_band, nz, nx, z1, z2, x1, x2);      // width-> z    height-> x            cutoff_x -> z    cutoff_z -> x   
...
void ELSRTM_ROT_EXP_IMAGE_SINGLESHOT(FD2DGPU_ELASTIC &fdtdgpu,const int sxnum,const int nDim,const int myid,float *record,float *record_x,int *gc,bool rbc,bool cpu_mem,int ntt,int iter, char* snapdir,\
...
//			fdtdgpu.normal_forward_and_born(direction);
			fdtdgpu.born_modeling(direction,0);
////////////////////////////////////////////////////////////			
			if(iter>=0){
				fdtdgpu.born_modeling(direction,3);			
				// R2Z<<<grid3,block3>>>(fdtdgpu.b_vx,fdtdgpu.b_vz,fft_vx,fft_vz,nxpml*nzpml);
				// CUFFT_CALL(cufftExecZ2Z(plan2D_forward, fft_vx, fft_vx, CUFFT_FORWARD));  
				// CUFFT_CALL(cufftExecZ2Z(plan2D_forward, fft_vz, fft_vz, CUFFT_FORWARD));  			        
				// GPU_lowpass<<<fdtdgpu.Grid_Block.grid,fdtdgpu.Grid_Block.block>>>(fft_vx,fft_vz,k_filter,nxpml,nzpml);
				// CUFFT_CALL(cufftExecZ2Z(plan2D_inverse, fft_vx, fft_vx, CUFFT_INVERSE));
				// CUFFT_CALL(cufftExecZ2Z(plan2D_inverse, fft_vz, fft_vz, CUFFT_INVERSE));			
				// GPU_divide<<<fdtdgpu.Grid_Block.grid,fdtdgpu.Grid_Block.block>>>(fft_vx,fft_vz,nxpml,nzpml);
				// Z2R<<<grid3,block3>>>(fdtdgpu.b_vx,fdtdgpu.b_vz,fft_vx,fft_vz,nxpml*nzpml);  
				fdtdgpu.born_modeling(direction,4);
			}
			fdtdgpu.born_modeling(direction,5);			// 2nd born approximate
///////////////////////////////////////////////////////////			
...
//					std::cout<<it<<std::endl;
		...
		//  	sprintf(tmps_bk, "%s/snap_FW_vz_it%d_%d_%d.dat",snapdir,it,fdtdgpu.nxpml,fdtdgpu.nzpml);
...
// 	sprintf(imagepath_pp, "%s/Lm_z_%d_%d_%d.dat",snapdir,iter,fdtdgpu.ntr,fdtdgpu.nt);
	FILE *fpp = NULL;
// 	if(sxnum==23){
...
// // output Lm 2nd
// 	cudaMemcpy(record_residual_z,fdtdgpu.GPUrecord_scatter2_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
// 	cudaMemcpy(record_residual_x,fdtdgpu.GPUrecord_scatter2_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
// 	sprintf(imagepath_pp, "%s/Lm2_z_%d_%d_%d.dat",snapdir,iter,fdtdgpu.ntr,fdtdgpu.nt);
// 	if(sxnum==23){
// 	fpp = fopen(imagepath_pp,"wb");
// 	fwrite(record_residual_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
// 	fclose(fpp);
...
// //
// //	Lm + Lm2nd - d 
// 	bool scatter2 = true;							// 2nd scatter subtract
...
// 	if(sxnum==23){
// 	sprintf(imagepath_pp, "%s/Lm+Lm2-d_z_%d_%d_%d.dat",snapdir,iter,fdtdgpu.ntr,fdtdgpu.nt);
// 	fpp = fopen(imagepath_pp,"wb");
// 	fwrite(record_residual_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
// 	fclose(fpp);	
// 	}
...
// ncc 
	...
	double *dx = new double[fdtdgpu.ntr];
	double *Dx = new double[fdtdgpu.ntr];
	double *dDx = new double[fdtdgpu.ntr];	
	double *dz = new double[fdtdgpu.ntr];
	double *Dz = new double[fdtdgpu.ntr];
	double *dDz = new double[fdtdgpu.ntr];	
	float *record_scatter_z = new float[fdtdgpu.ntr*fdtdgpu.nt];
	float *record_scatter_x = new float[fdtdgpu.ntr*fdtdgpu.nt];
	float *record_z_cpu = new float[fdtdgpu.ntr*fdtdgpu.nt];
	float *record_x_cpu = new float[fdtdgpu.ntr*fdtdgpu.nt];	

	memset(dx,0,fdtdgpu.ntr*sizeof(double));
	memset(Dx,0,fdtdgpu.ntr*sizeof(double));
	memset(dDx,0,fdtdgpu.ntr*sizeof(double));
	memset(dz,0,fdtdgpu.ntr*sizeof(double));
	memset(Dz,0,fdtdgpu.ntr*sizeof(double));
	memset(dDz,0,fdtdgpu.ntr*sizeof(double));		

	memset(record_scatter_z,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));
	memset(record_scatter_x,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));
	memset(record_z_cpu,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));
	memset(record_x_cpu,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));

	if(iter==0){
		// misfit = 0;			// in ncc 0 is max misfit value
		// bool scatter2 = false;
		// fdtdgpu.subtract(scale_size,scatter2);					// r  =  r - alpha*q
		// cudaMemcpy(record_residual_z,fdtdgpu.GPUrecord_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
		// cudaMemcpy(record_residual_x,fdtdgpu.GPUrecord_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);	
		...
		for(int i=0;i<fdtdgpu.ntr;i++)
		for(int j=0;j<fdtdgpu.nt;j++){
			Dx[i] += record_x_cpu[i*fdtdgpu.nt+j]*record_x_cpu[i*fdtdgpu.nt+j];
			Dz[i] += record_z_cpu[i*fdtdgpu.nt+j]*record_z_cpu[i*fdtdgpu.nt+j];	
		}	
		for(int i=0;i<fdtdgpu.ntr;i++)
		for(int j=0;j<fdtdgpu.nt;j++){
// using with method 1 (strength of amplitude normalized using scatter and record)
			record_residual_x[i*fdtdgpu.nt+j] = 1/(sqrt(Dx[i])*sqrt(Dx[i])) * (0.0f - record_x_cpu[i*fdtdgpu.nt+j]);
			record_residual_z[i*fdtdgpu.nt+j] = 1/(sqrt(Dz[i])*sqrt(Dz[i])) * (0.0f - record_z_cpu[i*fdtdgpu.nt+j]);			

// using with method 2 (strength of amplitude normalized only using record)
			// record_residual_x[i*fdtdgpu.nt+j] = 1/(sqrt(Dx[i])) * (0.0f - record_x_cpu[i*fdtdgpu.nt+j]);
			// record_residual_z[i*fdtdgpu.nt+j] = 1/(sqrt(Dz[i])) * (0.0f - record_z_cpu[i*fdtdgpu.nt+j]);

// using with method 3 (strength of amplitude not normalized)
			// record_residual_x[i*fdtdgpu.nt+j] = (0.0f - record_x_cpu[i*fdtdgpu.nt+j]);
			// record_residual_z[i*fdtdgpu.nt+j] = (0.0f - record_z_cpu[i*fdtdgpu.nt+j]);	

// using with method 4 (strength of amplitude normalized only using scatter)
			// record_residual_x[i*fdtdgpu.nt+j] = 1/(sqrt(Dx[i])) * (0.0f - record_x_cpu[i*fdtdgpu.nt+j]);
			// record_residual_z[i*fdtdgpu.nt+j] = 1/(sqrt(Dz[i])) * (0.0f - record_z_cpu[i*fdtdgpu.nt+j]);

		}	
		cudaMemcpy(fdtdgpu.GPUrecord_scatter_x,record_residual_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyHostToDevice);
		cudaMemcpy(fdtdgpu.GPUrecord_scatter_z,record_residual_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyHostToDevice);	
	}
	else{
		...
		// fclose(fpp);
		// sprintf(imagepath_pp, "%s/record_z_cpu_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
		// fpp = fopen(imagepath_pp,"wb");
		// fwrite(record_z_cpu,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
		// fclose(fpp);
		...
		// fclose(fpp);
		// sprintf(imagepath_pp, "%s/record_x_cpu_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
		// fpp = fopen(imagepath_pp,"wb");
		// fwrite(record_x_cpu,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
		// fclose(fpp);

		for(int i=0;i<fdtdgpu.ntr;i++)
		for(int j=0;j<fdtdgpu.nt;j++){
			dx[i] += record_scatter_x[i*fdtdgpu.nt+j]*record_scatter_x[i*fdtdgpu.nt+j];
			dz[i] += record_scatter_z[i*fdtdgpu.nt+j]*record_scatter_z[i*fdtdgpu.nt+j];		
			Dx[i] += record_x_cpu[i*fdtdgpu.nt+j]*record_x_cpu[i*fdtdgpu.nt+j];
			Dz[i] += record_z_cpu[i*fdtdgpu.nt+j]*record_z_cpu[i*fdtdgpu.nt+j];	
			dDx[i] += record_scatter_x[i*fdtdgpu.nt+j]*record_x_cpu[i*fdtdgpu.nt+j];
			dDz[i] += record_scatter_z[i*fdtdgpu.nt+j]*record_z_cpu[i*fdtdgpu.nt+j];	
			// if(i==100&&j==fdtdgpu.nt-1){
			// 	printf("i=100,record_scatter_x=%e,record_x_cpu=%e\n",record_scatter_x[i*fdtdgpu.nt+j],record_x_cpu[i*fdtdgpu.nt+j]);						
			// 	printf("i=100,dx=%e,dz=%e,Dx=%e,Dz=%e,dDx=%e,dDz=%e\n",dx[i],dz[i],Dx[i],Dz[i],dDx[i],dDz[i]);		
			// 	sleep(30);						
			// }
		...
		for(int i=0;i<fdtdgpu.ntr;i++)
		{
			misfit -= (1.0f/(2*fdtdgpu.ntr)) * (dDx[i]/(sqrt(dx[i])*sqrt(Dx[i])) + dDz[i]/(sqrt(dz[i])*sqrt(Dz[i])));						
		}	

		// std::cout<<"sxnum = "<<sxnum<<"\t misfit = "<<misfit<<std::endl;

		// printf("sxnum = %d,dx=%e,dz=%e,Dx=%e,Dz=%e,dDx=%e,dDz=%e\n",sxnum,dx[fdtdgpu.ntr/2],dz[fdtdgpu.ntr/2],Dx[fdtdgpu.ntr/2],Dz[fdtdgpu.ntr/2],dDx[fdtdgpu.ntr/2],dDz[fdtdgpu.ntr/2]);

		// printf("sxnum = %d,sqrt(dx[i])=%e,sqrt(Dx[i])=%e\n",sxnum,sqrt(dx[fdtdgpu.ntr/2]),sqrt(Dx[fdtdgpu.ntr/2]));

		// sleep(30);

		for(int i=0;i<fdtdgpu.ntr;i++)
		for(int j=0;j<fdtdgpu.nt;j++){
// strength of amplitude normalized using scatter and record	
			record_residual_x[i*fdtdgpu.nt+j] = 1/(sqrt(dx[i])*sqrt(Dx[i])) * ((dDx[i]/dx[i]) * record_scatter_x[i*fdtdgpu.nt+j] - record_x_cpu[i*fdtdgpu.nt+j]);
			record_residual_z[i*fdtdgpu.nt+j] = 1/(sqrt(dz[i])*sqrt(Dz[i])) * ((dDz[i]/dz[i]) * record_scatter_z[i*fdtdgpu.nt+j] - record_z_cpu[i*fdtdgpu.nt+j]);			

// strength of amplitude normalized only using record	
			// record_residual_x[i*fdtdgpu.nt+j] = 1/(sqrt(Dx[i])) * ((dDx[i]/dx[i]) * record_scatter_x[i*fdtdgpu.nt+j] - record_x_cpu[i*fdtdgpu.nt+j]);
			// record_residual_z[i*fdtdgpu.nt+j] = 1/(sqrt(Dz[i])) * ((dDz[i]/dz[i]) * record_scatter_z[i*fdtdgpu.nt+j] - record_z_cpu[i*fdtdgpu.nt+j]);		

// strength of amplitude  not normalized
			// record_residual_x[i*fdtdgpu.nt+j] =  ((dDx[i]/dx[i]) * record_scatter_x[i*fdtdgpu.nt+j] - record_x_cpu[i*fdtdgpu.nt+j]);
			// record_residual_z[i*fdtdgpu.nt+j] =  ((dDz[i]/dz[i]) * record_scatter_z[i*fdtdgpu.nt+j] - record_z_cpu[i*fdtdgpu.nt+j]);		

// strength of amplitude normalized only using scatter	
			// record_residual_x[i*fdtdgpu.nt+j] = 1/(sqrt(dx[i])) * ((dDx[i]/dx[i]) * record_scatter_x[i*fdtdgpu.nt+j] - record_x_cpu[i*fdtdgpu.nt+j]);
			// record_residual_z[i*fdtdgpu.nt+j] = 1/(sqrt(dz[i])) * ((dDz[i]/dz[i]) * record_scatter_z[i*fdtdgpu.nt+j] - record_z_cpu[i*fdtdgpu.nt+j]);				
...
// g = LT *(Lm - d)
	...
	stack = true;	// TODO:  if first iteration need to stack .
...
// backward wavefield
			...
			fdtdgpu.calculateTxz(direction_rec,backward);
			fdtdgpu.calculateTxxzz(direction_rec,backward);
			fdtdgpu.calculateVz(direction_rec,backward); 				
			fdtdgpu.calculateVx(direction_rec,backward);

			fdtdgpu.pure_born_reconstruct_backward(direction_rec,backward);			//receiver-side scatter wavefield
...
//	Lm 
	...
	// FILE *fpp;
	// char imagepath_pp[1024];
	// char snapdir[1024] = "/public/home/wangmq/prismatic/2nd/image_ncc_iter50_2pointborn_onlyforward_test";

 	// cudaMemcpy(record_scatter_z,fdtdgpu.GPUrecord_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
	// cudaMemcpy(record_scatter_x,fdtdgpu.GPUrecord_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);	

	// sprintf(imagepath_pp, "%s/Lm_x_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);
	// sprintf(imagepath_pp, "%s/Lm_z_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);

 	// cudaMemcpy(record_scatter_z,fdtdgpu.GPUrecord_scatter2_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
	// cudaMemcpy(record_scatter_x,fdtdgpu.GPUrecord_scatter2_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);	

	// sprintf(imagepath_pp, "%s/Lm2_x_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);
	// sprintf(imagepath_pp, "%s/Lm2_z_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);
...
// alpha = gT*g/(gT*LTLg) = (LT*delta_d)T*(LT*delta_d)/() = delta_dT*(L*LT*delta_d)/()
	...
	// sprintf(imagepath_pp, "%s/Lg_x_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);
	// sprintf(imagepath_pp, "%s/Lg_z_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);
	...
	// sprintf(imagepath_pp, "%s/Lg2_x_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);
	// sprintf(imagepath_pp, "%s/Lg2_z_%d_%d_%d_%d.dat",snapdir,iter,sxnum,fdtdgpu.ntr,fdtdgpu.nt);
	// fpp = fopen(imagepath_pp,"wb");
	// fwrite(record_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),1,fpp);
	// fclose(fpp);
